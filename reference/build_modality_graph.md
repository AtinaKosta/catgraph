...
category level, encoded as `"variable=level"`, and each edge represents
the pairwise association between two modalities from different
variables.
...
- remove_na:
  ...
  Logical. If TRUE, rows with missing values are removed before graph
  construction. Default is TRUE.

- min_count:

  Integer. Modalities with total count strictly below this threshold are
  removed before association computation. Default is 1.

## Value

An object of class `"catmodgraph"`, containing:
...
- `modalities`:
  ...
  A data frame describing node identities.

- `indicator_matrix`:

  The binary modality indicator matrix.
...
co-occurrence edges in the intended network.

**Interpretation.** This graph is descriptive and exploratory. It is not
a causal graph, not a respondent-segmentation model, and not a
conditional-independence graph. Because edge weights are absolute
values, users should inspect `std_resid` or use signed plotting options
when distinguishing attraction from repulsion matters.

**Implementation.** Since v0.8.0 the per-pair 2x2 statistics are
computed in vectorised closed form from the modality co-occurrence
matrix \\M^\top M\\, rather than by an inner loop over
[`chisq.test`](https://rdrr.io/r/stats/chisq.test.html) calls. For \\m\\
modalities on \\n\\ rows the dominant cost drops from \\O(m^2 n)\\ plus
a per-pair `chisq.test` overhead to a single \\O(m^2 n)\\ matrix product
plus \\O(m^2)\\ arithmetic. Typical speed-up on realistic data (\\m
\approx 40\\, \\n \approx 5000\\) is 30–100x. Numeric output is
algebraically identical to the earlier loop implementation up to
floating-point tolerance; edge ordering in the returned graph may
differ.

This function is the modality-level counterpart to
[`catgraph()`](https://atinakosta.github.io/catgraph/reference/catgraph.md).
...
#> [10] Class=2nd --Age=Adult   Class=3rd --Age=Adult   Class=Crew--Age=Adult  
#> [13] Sex=Female--Age=Adult   Sex=Male  --Age=Adult   Class=1st --Age=Child  
#> [16] Class=2nd --Age=Child   Class=3rd --Age=Child   Class=Crew--Age=Child  
#> [19] Sex=Female--Age=Child   Sex=Male  --Age=Child   Class=1st --Survived=No
